...
export function getTokenTotals(db: Database, sessionIds: string[]): TokenTotals {
  if (sessionIds.length === 0) {
    return {
      ...
      totalTokensReasoning: 0, totalTokensCacheRead: 0, totalTokensCacheWrite: 0,
      totalTokens: 0, totalMessages: 0,
    }
  }
  const placeholders = sessionIds.map(() => "?").join(",")
  const row = db.query<{
    cost: number; input: number; output: number; reasoning: number;
    cache_read: number; cache_write: number; total: number
  }, string[]>(`
    SELECT
      SUM(cost) as cost,
  ...
  const msgRow = db.query<{ cnt: number }, string[]>(`
...
export function getByAgentModel(db: Database, sessionIds: string[]): AgentModelRow[] {
  ...
  const placeholders = sessionIds.map(() => "?").join(",")
  return db.query<AgentModelRow, string[]>(`
    SELECT
...
export function getCacheEfficiency(db: Database, sessionIds: string[]): CacheEfficiencyRow[] {
  if (sessionIds.length === 0) return []
  const placeholders = sessionIds.map(() => "?").join(",")
  const rows = db.query<{ model: string; cache_read: number; input: number }, string[]>(`
    SELECT
      COALESCE(model, 'unknown') as model,
      SUM(tokens_cache_read) as cache_read,
      SUM(tokens_input) as input
...
export function getCostPer1k(db: Database, sessionIds: string[]): CostPer1kRow[] {
  if (sessionIds.length === 0) return []
  const placeholders = sessionIds.map(() => "?").join(",")
  const rows = db.query<{ model: string; cost: number; tokens: number }, string[]>(`
    SELECT
      COALESCE(model, 'unknown') as model,
      SUM(cost) as cost,
      SUM(tokens_input + tokens_output + tokens_reasoning + tokens_cache_read) as tokens
...
export function getAgentDelegation(db: Database, sessionIds: string[]): AgentDelegationRow[] {
  ...
  return db.query<AgentDelegationRow, string[]>(`
    SELECT
...
export function getSessionMeta(db: Database, sessionId: string): SessionMeta | null {
  const session = db.query<{
    ...
    tokens_reasoning: number; tokens_cache_read: number; tokens_cache_write: number
  }, [string]>(`
  ...
  const msgCounts = db.query<{ role: string; cnt: number }, [string]>(`
  ...
  const agentRows = db.query<{ agent: string; cnt: number }, [string]>(`
  ...
  const modelRows = db.query<{ model: string; cnt: number }, [string]>(`
  ...
  return {
    ...
    totalTokens: session.tokens_input + session.tokens_output + session.tokens_reasoning + session.tokens_cache_read,
    cost: session.cost,
    toolCounts,
    agentCounts,
    modelCounts,
