...
function makeReportData(partial: Partial<ReportData> = {}): ReportData {
  return {
    ...
    facets: new Map(),
    aggregates: {
      project_areas: { areas: [{ name: "Backend", description: "API work", session_count: 3, example_goals: ["fix auth"] }] },
      friction: { intro: "Some friction", categories: [{ category: "misunderstood_request", description: "Agent misunderstood you", examples: ["example 1"] }] },
      suggestions: {
        agents_md_additions: [{ rule: "Always run tests", rationale: "consistency", why_now: "repeated 3x" }],
        features_to_try: [{ feature: "Skills", one_liner: "Reusable prompts", why_for_you: "saves time", example: "opencode skills" }],
        workflow_patterns: [{ pattern: "TDD", benefit: "fewer bugs", how_to: "write tests first", copyable_prompt: "Write a test for..." }]
      },
      interaction_style: {},
      agent_performance: {},
      tool_health: {},
...
describe("generateReport", () => {
  ...
  it("JSON island parses back correctly", () => {
    ...
    const match = html.match(/<script type="application\/json" id="insights-data">([\s\S]*?)<\/script>/)
