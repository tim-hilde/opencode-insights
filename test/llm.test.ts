...
import { runLlm, extractJson, mapLimit, JsonParseError } from "../src/llm.ts"
import type { LlmClient, LlmCallOptions } from "../src/llm.ts"
...
describe("runLlm", () => {
  ...
  it("passes system prompt when provided", async () => {
    ...
    const client = makeMockClient("ok")
    ;(client.session as any).prompt = async (opts: { path: { id: string }; body: { system?: string } }) => {
