...
import { openDb, resolveDbPath, listSessionIds, getTokenTotals, getByAgentModel, getToolErrorRates, getCacheEfficiency, getCostPer1k, getAgentDelegation, getPartsWithMessages } from "../src/db.ts"
import { filterSessions, aggregateAll, reconstructTranscript } from "../src/extract.ts"
...
try {
  ...
  console.log(`  input=${totals.totalTokensInput}, output=${totals.totalTokensOutput}, cost=$${totals.totalCost.toFixed(4)}`)
  ...
  for (const row of errorRates.slice(0, 5)) {
    console.log(`  ${row.tool}: ${row.totalCalls} calls, ${(row.errorRate * 100).toFixed(1)}% errors`)
  }

  if (errorRates.length === 0) {
    console.log("  WARNING: No tool data found. Checking part data shape...")
    // Debug: check what $.type values exist
    const sample = db.query<{ types: string }, []>(
    ...
    const toolSample = db.query<{ tool_name: string; tool_raw: string }, []>(
  ...
  console.log(`Stats: ${stats.totalSessions} sessions, $${stats.totalCost.toFixed(4)} cost, ${stats.totalTokens} tokens`)
  console.log(`Date range: ${new Date(stats.dateRange.from).toLocaleDateString()} – ${new Date(stats.dateRange.to).toLocaleDateString()}`)
  console.log(`Top tools: ${stats.topTools.slice(0, 3).map(t => `${t.tool}(${t.count})`).join(", ")}`)
  console.log(`Top agents: ${stats.topAgents.slice(0, 3).map(a => `${a.agent}(${a.count})`).join(", ")}`)
